#!/usr/bin/env python3
"""Ling-3.0-tiny vs Qwen3.6-35B-A3B 全面对比评测。
...
用法: python benchmarks/eval_ling_vs_qwen.py
"""
import asyncio
import json
import re
...
def check_answer(name: str, predicted: str, item: dict) -> bool:
    """检查答案是否正确。"""
    if name == "mmlu":
        # answer 是数字索引 0/1/2/3，predicted 是 A/B/C/D
        try:
...
async def main():
    print("=" * 70)
    print("Ling-3.0-tiny vs Qwen3.6-35B-A3B 全面对比评测")
    print(f"每数据集采样: {SAMPLE_SIZE} 题 | 温度: 0 (greedy)")
    print("=" * 70)
    ...
    for bench in BENCHMARKS:
        ...
        for model in MODELS:
            ...
            if result:
                speeds.append(f"{result['avg_tok_s']:.1f}")
            else:
                speeds.append("N/A")
        print(f"  {bench:<14}: {' vs '.join(speeds)}")

    # 保存 JSON
    output_path = Path(__file__).parent / "eval_ling_vs_qwen_results.json"
