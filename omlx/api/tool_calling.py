...
def _serialize_tool_call_arguments(arguments: Any) -> str:
    ...
    it, clean any quoted-key artifacts, and re-serialize.
    """
    ...
    if isinstance(arguments, dict):
        # The mlx-vlm Gemma 4 parser may hand back keys wrapped in embedded
        # quotes ('"key"'); clean them so the echoed arguments stay usable.
    ...
    if isinstance(arguments, str):
        ...
        if isinstance(parsed, dict):
            if _has_quoted_keys(parsed):
                parsed = _clean_quoted_keys(parsed)
