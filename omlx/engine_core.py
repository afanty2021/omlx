    ...
    async def generate(
        ...
        **kwargs,
        ...
        while retry_count < max_retries:
            ...
            # stream_outputs(): the orphan reaper is pop-only and may drop the
            # dict entry once the request is finished, but a held reference
            # still drains. Re-fetching after the await would race the reaper
            # if this coroutine is starved past the grace window under heavy
            # load (#1154).
            collector = self._output_collectors.get(request_id)
            if collector is None:
